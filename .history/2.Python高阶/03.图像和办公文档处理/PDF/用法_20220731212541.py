...
# 7.加密PDF文件
pdfReader = PyPDF2.PdfFileReader(open('./File/watermarked.pdf', 'rb'))
pdfWriter = PyPDF2.PdfFileWriter()
...
# 8.删除PDF文件中的页面
pdfReader = PyPDF2.PdfFileReader(open('./File/combine.pdf', 'rb'))
pdfWriter = PyPDF2.PdfFileWriter()
for pageNum in range(pdfReader.numPages):
    pageObj = pdfReader.getPage(pageNum)
    pdfWriter.addPage(pageObj)
    # 删除第一页
    if pageNum == 0:
        pdfWriter.removePage(0)
# 写入文件
with open('./File/conbine_delete.pdf', 'wb') as f:
    pdfWriter.write(f)
