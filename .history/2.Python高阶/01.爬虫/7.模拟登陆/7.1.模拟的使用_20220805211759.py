...
class Login(object):
    ...
    def __init__(self):
        ...
        self.explore = 'https://github.com/explore'
    ...
    def login(self, email, password):
        ...
        response = self.session.get(self.explore, headers=self.headers)
        if response.status_code == 200:
            self.explore(response.text)
    ...
    def explore(self, html):
        selector = etree.HTML(html)
        projects = selector.xpath('//div[contains(@class, "col-12")]')
        for item in projects:
            name = item.xpath('.//a[contains(@class, "muted-link")]/text()')[0]
            print(name)
