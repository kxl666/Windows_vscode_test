...
class CrackGeetest():
    ...
    def get_screenshot(self):
        ...
        :return: 截图对象
        """
        screenshot = self.browser.get_screenshot_as_png()  # 获取网页截图
        screenshot = Image.open(BytesIO(screenshot))  # 转换成PIL图片
        return screenshot
    ...
    def get_geetest_image(self, name='captcha.png'):
        ...
        captcha = screenshot.crop((left, top, right, bottom))  # 剪切图片
        captcha.save(name)
        return captcha
    ...
    def move_to_gap(self, slider, track):
        ...
        :return:
        """
        ActionChains(self.browser).click_and_hold(slider).perform()  # 点击并按住滑块
        for x in track:
            ActionChains(self.browser).move_by_offset(
                xoffset=x, yoffset=0).perform()  # 移动滑块
        time.sleep(0.5)
